...
M=[m1*l2^2+m2*l1^2+I -sin(theta1-theta2)*m1*l2*l3 -(m1*l2+m2*l1)*sin(theta1) 0 0;
-sin(theta1-theta2)*m1*l2*l3 m1*l3^2 m1*l3*cos(theta2) 0 0;
-(m1*l2+m2*l1)*sin(theta1) m1*l3*cos(theta2) m1+m2+m3 0 0;
0 0 0 m4 0;
0 0 0 0 m4]
...
0 0 0 0 1]
	...
	f=[-cos(theta1-theta2)*m1*l2*l3*dtheta2^2-m1*g*l2*cos(theta1)-m2*g*l1*cos(theta1)
	    ...
	    (m1*l2+m2*l1)*cos(theta1)*dtheta1^2+m1*l3*sin(theta2)*dtheta2^2
	    0
	    -m4*g]
